...
import { ManagerTask } from '../schemas/manager-task.schema';
import { QueryParams, ResponseQuery } from 'src/interface/i-base-db-service';
import { MAX_ITEM_QUERYS } from 'src/const';

@Injectable()
export class ManagerTaskDBService extends BaseDBService<ManagerTask> {

  constructor(@InjectModel(ManagerTask.name) private readonly entityModel) {
    super(entityModel);
  }
  
  async CURDTask(entity: any): Promise<ResponseQuery<any>> {
