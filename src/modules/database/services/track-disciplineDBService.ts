...
import { TrackDisciplineBook } from '../schemas/track-discipline-book.schema';
import { UnitDBService } from './unitDBService';

@Injectable()
export class ManagerTrackDisciplineDBService extends BaseDBService<TrackDisciplineBook> {

  @Inject(UnitDBService)
  unitDBService: UnitDBService;
  
  constructor(@InjectModel(TrackDisciplineBook.name) private readonly entityModel) {
    super(entityModel);
  }

  async getTrackDisciplineBelongUnit(
    userUnitId: string,
    unitId: string,
    query: QueryParams,
  ...
  async CURDTrackDiscipline(entity: any): Promise<ResponseQuery<any>> {
