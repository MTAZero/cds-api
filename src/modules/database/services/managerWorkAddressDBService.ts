...
import { ManagerWorkAddress } from '../schemas/manager-work-address.schema';
import { QueryParams, ResponseQuery } from 'src/interface/i-base-db-service';
import { MAX_ITEM_QUERYS } from 'src/const';

@Injectable()
export class ManagerWorkAddressDBService extends BaseDBService<ManagerWorkAddress> {

  constructor(@InjectModel(ManagerWorkAddress.name) private readonly entityModel) {
    super(entityModel);
  }
  
  async CURDWorkAddress(entity: any): Promise<ResponseQuery<any>> {
